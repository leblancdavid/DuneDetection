...
namespace dune
{
	EndpointSegmentLinker::EndpointSegmentLinker()
	{
	...
	EndpointSegmentLinker::EndpointSegmentLinker(const SegmentLinkParameters &params)
	{
		Parameters = params;
	}

	EndpointSegmentLinker::EndpointSegmentLinker(const EndpointSegmentLinker &cpy)
	{
		cpy.Parameters;
	...
	std::vector<DuneSegment> EndpointSegmentLinker::GetLinkedSegments(const std::vector<DuneSegment> &inputs)
	{
		//This will init the distance table and the link tables, which allows us to find the links between segments
		InitTables(inputs, Parameters);
	...
	DuneSegment EndpointSegmentLinker::Link(const DuneSegment &seg1, const DuneSegment &seg2, LinkingMethod method, LinkType type)
	{
	...
	DuneSegment EndpointSegmentLinker::LinkUsingEndPointToEndPoint(const DuneSegment &seg1, const DuneSegment &seg2, LinkType type)
	{
	...
	void EndpointSegmentLinker::InitTables(const std::vector<DuneSegment> &inputs, const SegmentLinkParameters &params)
	{
	...
	void EndpointSegmentLinker::UpdateLinkTable()
	{
		//For each segment
	...
	std::vector<DuneSegment> EndpointSegmentLinker::GetLinkedSegmentFromTables(const std::vector<DuneSegment> &inputs)
	{
		//Search each segment
