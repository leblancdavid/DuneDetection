...
#ifndef _TVTERMREADMEMORY_H_
#define _TVTERMREADMEMORY_H_
...
namespace TVF {
	namespace ProcessTools {
		namespace Visitors {
			...
			*/
			template <class T>
			class TVTermReadMemory : public ITVTerminal<T> {
			protected:
				/***************
				* Attributes: *
				***************/
			...
			public:
				...
				************************************/
				...
				virtual void plotRemitter(TVRETensor2<T> *rem) {
					// Saving:
					vector_dim_t dim = rem->getDim();
					tns_copy_from_matrix(rem->getTensor(),_pos,dim);
				...
				**************/
				...
				TVTermReadMemory<T> *setBuffer(T *buf) {
					// Saving:
					_buf = buf;
...
#endif //ndef _TVTERMREADMEMORY_H_
